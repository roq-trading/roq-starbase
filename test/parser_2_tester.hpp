...
template <typename T>
struct Parser2Tester final : public protocol::sbe::Parser2::Handler {
  using value_type = std::remove_cvref_t<T>;
  using callback_type = std::function<void(value_type const &, deribit::sbe::market_data::MdMessageHeader const &, protocol::sbe::PacketHeader const &)>;
  ...
  static void dispatch(callback_type const &callback, auto const &message) {
    ...
    auto res = protocol::sbe::Parser2::dispatch(handler, std::span{reinterpret_cast<std::byte const *>(std::data(message)), std::size(message)}, {});
    CHECK(res == true);
 ...
 protected:
  ...
  bool operator()(protocol::sbe::PacketHeader const &) override { return true; }
  //
  void operator()(
