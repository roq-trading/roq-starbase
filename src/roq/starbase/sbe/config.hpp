...
template <>
struct fmt::formatter<roq::starbase::sbe::Config::Connection> {
  constexpr auto parse(format_parse_context &context) { return std::begin(context); }
  auto format(roq::starbase::sbe::Config::Connection const &value, format_context &context) const {
...
template <>
struct fmt::formatter<roq::starbase::sbe::Config> {
  constexpr auto parse(format_parse_context &context) { return std::begin(context); }
  auto format(roq::starbase::sbe::Config const &value, format_context &context) const {
    using namespace std::literals;
    return fmt::format_to(
        context.out(),
        R"({{)"
